...
#include "thirdparty/source-sdk-stubs/mathlib/vector2d.h"
...
public:
	...
	Vector(const char* string_representation);
...
#define USE_M64S ( ( !defined( _X360 ) ) && ( ! defined( __linux__) ) )
...
inline Vector::Vector(const char* string_representation)
{
	if (!string_representation || !(*string_representation)) {
		print(Warning, "%s Failed to read string representation; "
			"initializing a zeroed vector, instead.", __FUNCTION__);
		x = 0; y = 0; z = 0;
	}
	else {
		std::string search = std::string(string_representation);
		const std::string delimiter = " ";

		constexpr size_t xyz_num_elements = 3;
		float xyz[xyz_num_elements]{ 0 };

		std::string token;
		for (size_t pos = 0, i = 0; (pos = search.find(delimiter)) != std::string::npos; ) {
			token = search.substr(0, pos);
			xyz[i] = static_cast<float>(atof(token.c_str()));
			search.erase(0, pos + delimiter.length());
			++i;
			if (i >= xyz_num_elements) {
				print(Warning, "%s Iterated too many elements in Vector ctor \"%s\"; "
					"expected a space-delimited format of: \"1.0 2.0 3.0\"",
					__FUNCTION__, string_representation);
				break;
			}
		}
		x = xyz[0]; y = xyz[1];
		z = static_cast<float>(atof(search.c_str()));
	}
}
...
{
	CHECK_VALID(v);
	return (vec_t)fast_sqr(v.x*v.x + v.y*v.y + v.z*v.z);		
}
...
{
	CHECK_VALID(*this);
	return (vec_t)fast_sqr( LengthSqr( ) );
}
